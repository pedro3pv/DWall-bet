...
(defn calculate-moneyline-away [list_lines]
  (if (= (get-in list_lines [:moneyline :moneyline_away]) 1.0E-4)
    (int 0)
    (let [value (int (get-in list_lines [:moneyline :moneyline_away]))]
      (cond
...
(defn calculate-moneyline-home [list_lines]
  (if (= (get-in list_lines [:moneyline :moneyline_home]) 1.0E-4)
    (int 0)
    (let [value (int (get-in list_lines [:moneyline :moneyline_home]))]
      (cond
        (< value 0) (inc (double (abs (/ 100 value))))
        (> value 0) (double (/ value 100))))))
...
(defn calculate-odd [aposta api_result]
  (let [list_lines (get-in aposta [:event-details :lines :selected_affiliate])
        mercado (int (get-in aposta [:mercado]))
        resultado_home (int (get-in api_result [:score :winner_home]))
        resultado_away (int (get-in api_result [:score :winner_away]))
        pontos_home (int (get-in api_result [:score :score_home]))
        pontos_away (int (get-in api_result [:score :score_away]))
        handicap-casa (get-in aposta [:event-details :lines :spread :point_spread_home])
        handicap-fora (get-in aposta [:event-details :lines :spread :point_spread_away])
        odds-casa (get-in aposta [:event-details :lines :spread :point_spread_home_money])
        odds-fora (get-in aposta [:event-details :lines :spread :point_spread_away_money])]
    (cond
